...
extension DispatchQueue {
    ...
    public func async(after timeInterval: TimeInterval, execute work: @escaping @convention(block) () -> ()) {
        let milliseconds = Int(timeInterval * 1000)
        let deadline = DispatchTime.now() + .milliseconds(milliseconds)
        asyncAfter(deadline: deadline, execute: work)
    }
    
    public func async(afterWall timeInterval: TimeInterval, execute work: @escaping @convention(block) () -> ()) {
        let milliseconds = Int(timeInterval * 1000)
